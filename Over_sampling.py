...
#Over-sampling with smotetek only
'''
...
print("Après undersampling:", Counter(y_resampled))
'''
# Étape 2: Oversampling

ros = RandomOverSampler(sampling_strategy=1.0, random_state=42)
X_resampled, y_resampled = ros.fit_resample(X_train, Y_train)
print("Après oversampling:", Counter(y_resampled))

# Puis entraine ton modèle :
...
model_class_weights.fit(X_resampled, y_resampled)


y_pred_proba = model_class_weights.predict_proba(X_test)[:, 1]

for seuil in [0.5, 0.55, 0.6, 0.65,0.7,0.75]:
