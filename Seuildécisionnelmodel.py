...
y_probs_train = catboost_model.predict_proba(X_train)[:, 1]  # Probabilité pour la classe fraude (1)
y_probs_test = catboost_model.predict_proba(X_test)[:, 1]    # Idem pour le test

# Tester plusieurs seuils
seuils = [0.3, 0.4, 0.5, 0.6, 0.7]

print("Seuil | Precision_Train | Recall_Train | F1_Train | Precision_Test | Recall_Test | F1_Test")
print("-" * 90)

for seuil in seuils:
    # Appliquer le seuil
    y_train_pred_adj = (y_probs_train >= seuil).astype(int)
    y_test_pred_adj = (y_probs_test >= seuil).astype(int)

    # Calculer les métriques
    precision_train = precision_score(Y_train, y_train_pred_adj)
    recall_train = recall_score(Y_train, y_train_pred_adj)
    f1_train = f1_score(Y_train, y_train_pred_adj)

    precision_test = precision_score(Y_test, y_test_pred_adj)
    recall_test = recall_score(Y_test, y_test_pred_adj)
    f1_test = f1_score(Y_test, y_test_pred_adj)

    # Afficher les résultats
    print(f"{seuil:.2f}  |  {precision_train:.4f}  |  {recall_train:.4f}  |  {f1_train:.4f}  |  "
          f"{precision_test:.4f}  |  {recall_test:.4f}  |  {f1_test:.4f}")
